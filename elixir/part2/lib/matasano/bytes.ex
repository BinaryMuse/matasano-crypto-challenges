defmodule Matasano.Bytes do
  ...
  ## Examples
      ...
      '010203fdfeff'
  """
  def to_hex(bytes) when is_binary(bytes), do: :binary.bin_to_list(bytes) |> to_hex |> :binary.list_to_bin

  def to_hex(bytes) when is_list(bytes) do
    hexes = Enum.map bytes, fn(byte) -> :string.right(Integer.to_char_list(byte, 16), 2, ?0) end
  ...
  ## Examples
      ...
      [1,2,3,253,254,255]
  """
  def from_hex(hex) when is_binary(hex), do: from_hex(:binary.bin_to_list(hex)) |> :binary.list_to_bin

  def from_hex(hex) when is_list(hex) do
    hex |> split_into(2) |> Enum.map(&List.to_integer(&1, 16))
  end
  ...
  def split_into(binary, size) when is_binary(binary) do
    binary |> :binary.bin_to_list |> split_into(size) |> Enum.map(&:binary.list_to_bin(&1))
  end
  ...
  def xor_sum(binary1, binary2) when is_binary(binary1) and is_binary(binary2) do
    xor_sum(:binary.bin_to_list(binary1), :binary.bin_to_list(binary2)) |> :binary.list_to_bin
  end
  ...
  @doc """
  Pad the binary or list using PKCS#7 padding to the given length.

  ## Examples

      iex> Matasano.Bytes.pad_to("YELLOW SUBMARINE", 20)
      "YELLOW SUBMARINE\x04\x04\x04\x04"
  """
  def pad_to(bytes, length) when is_binary(bytes) do
    remaining = length - byte_size(bytes)
    padding = List.duplicate(remaining, remaining) |> :binary.list_to_bin
    bytes <> padding
  end

  def pad_to(bytes, length) when is_list(bytes) do
    bytes |> :binary.list_to_bin |> pad_to(length) |> :binary.bin_to_list
  end
  ...
  def hamming_distance(a, b) when is_binary(a) and is_binary(b) do
    hamming_distance(:binary.bin_to_list(a), :binary.bin_to_list(b))
  end
  ...
  def transpose(list) when is_list(list) do
    longest = Enum.max_by list, fn(l) -> length l end
