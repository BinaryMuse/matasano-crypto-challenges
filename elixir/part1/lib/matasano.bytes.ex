defmodule Matasano.Bytes do
  ...
  ## Examples
      ...
      '010203fdfeff'
  """
  def to_hex(bytes) when is_binary(bytes), do: :binary.bin_to_list(bytes) |> to_hex |> List.to_string

  def to_hex(bytes) when is_list(bytes) do
    hexes = Enum.map bytes, fn(byte) -> :string.right(Integer.to_char_list(byte, 16), 2, ?0) end
    Enum.join(hexes, "") |> String.downcase |> String.to_char_list
  end
  ...
  def xor_sum(binary1, binary2) when is_binary(binary1) and is_binary(binary2) do
    xor_sum(:binary.bin_to_list(binary1), :binary.bin_to_list(binary2)) |> List.to_string
  end
